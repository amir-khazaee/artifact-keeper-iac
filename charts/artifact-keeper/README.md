...
| dependencyTrack | object | `{"adminPassword":"","affinity":{},"bootstrap":{"containerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]}},"enabled":false,"image":{"pullPolicy":"IfNotPresent","repository":"badouralix/curl-jq","tag":"latest"},"podSecurityContext":{"fsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}},"containerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"readOnlyRootFilesystem":true},"database":{"existingSecret":"","secretKeys":{"host":"host","jdbcUri":"jdbc-uri","password":"password","port":"port","username":"username"}},"enabled":true,"image":{"repository":"dependencytrack/apiserver","tag":"4.11.4"},"initContainerSecurityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"readOnlyRootFilesystem":true},"nodeSelector":{},"persistence":{"size":"5Gi","storageClass":""},"podSecurityContext":{"fsGroup":1000,"runAsNonRoot":true,"runAsUser":1000},"resources":{"limits":{"cpu":"2","ephemeral-storage":"1Gi","memory":"6Gi"},"requests":{"cpu":"500m","ephemeral-storage":"256Mi","memory":"4Gi"}},"tmpSizeLimit":"2Gi","tolerations":[],"topologySpreadConstraints":[]}` | DependencyTrack SBOM analysis Provides SBOM ingestion, license analysis, and vulnerability correlation. Requires significant memory (4Gi+) to load its internal vulnerability database on startup. The bootstrap init container creates the initial admin user and API key for backend integration. |
| dependencyTrack.bootstrap.enabled | bool | `false` | Run the one-shot DependencyTrack bootstrap Job. Disabled by default because the Job requires a non-empty `dependencyTrack.adminPassword` (enforced by validateSecrets); enable it together with that password. |
| dependencyTrack.bootstrap.image | object | `{"pullPolicy":"IfNotPresent","repository":"badouralix/curl-jq","tag":"latest"}` | Image for the one-shot DependencyTrack bootstrap Job. It MUST already contain `curl` and `jq`. The bootstrap pod runs non-root (see podSecurityContext below), so installing packages at runtime with `apk add` fails ("Unable to lock database: Permission denied"); shipping the tools in the image avoids any runtime package install or network egress. Pin to a digest or mirror this into your own registry for production. |
